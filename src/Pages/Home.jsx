        ...
        <div className="flex flex-wrap md:gap-2">
          {cards.map((card, index) => (
            <div
              ...
              className={`h-28 ${
                activeMenu
                  ? "w-full lg:w-[45%] xl:w-[216px]"
                  : "w-full md:w-[45%] lg:w-[430px] xl:w-[272px]"
              } p-4 rounded-lg bg-white dark:bg-secondary-dark-bg m-2 md:ml-5 xl:m-2 xl:ml-4 drop-shadow-sm flex items-center gap-4`}
            >
              ...
              <div className="flex flex-col w-[70%] text-primry-color dark:text-white">
                ...
                <div
                  className={`flex flex-col ${
                    !activeMenu && "lg:flex-row xl:flex-col lg:justify-between "
                  } `}
                >
                  <span className="font-extrabold ">+ {card.valeu}</span>
                  <span
                    className={`flex items-center ${
                      card.up ? "text-secondary-color" : "text-tertiary-color"
                    }`}
                  >
                    {card.up ? upIcon : downIcon}
                    <span className="ml-1">{card.stat}%</span>
                  </span>
                </div>
              </div>
            </div>
